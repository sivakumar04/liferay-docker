...
function generate_thread_dump {
	mkdir --parents "${LIFERAY_SRE_THREAD_DUMPS_DIRECTORY}"

	local file_name="${LIFERAY_SRE_THREAD_DUMPS_DIRECTORY}/$(hostname)_$(date +'%Y-%m-%d_%H-%M-%S').tdump"
...
function main {
	...
	do
		...
		then
			if [ "${started}" == "true" ]
			then
				if [ "${LIFERAY_SRE_TOMCAT_THREAD_ACTIVE_COUNT_ENABLED}" == "true" ] && [ -n "${LIFERAY_SRE_TOMCAT_THREAD_ACTIVE_COUNT_THRESHOLD}" ]
				then
					...
					if [[ "${tomcat_thread_active_count}" =~ ^[0-9]+$ ]] && [ "${tomcat_thread_active_count}" -ge "${LIFERAY_SRE_TOMCAT_THREAD_ACTIVE_COUNT_THRESHOLD}" ]
					then
